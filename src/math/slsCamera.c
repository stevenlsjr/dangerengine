...
void sls_camera_resize(slsCamera *self, int x, int y)
{
  ...
  if (self->is_perspective) {
    kmMat4PerspectiveProjection(&self->frustrum, self->perspective.fov, aspect,
                                self->perspective.near, self->perspective.far);
  } else {
    ...
    kmMat4OrthographicProjection(
        &self->frustrum, left * h_width, right * h_width, down * h_height,
        up * h_height, self->ortho.near, self->ortho.far);
  }
}
