...
{
  vec_loader loader;
  loader = (vec_loader){ { in->arr[0].x, in->arr[1].x, in->arr[2].x,
                           in->arr[3].x } };
  out->xv = _mm_load_ps(loader.vec);

  loader = (vec_loader){ { in->arr[0].y, in->arr[1].y, in->arr[2].y,
                           in->arr[3].y } };
  out->yv = _mm_load_ps(loader.vec);

  loader = (vec_loader){ { in->arr[0].z, in->arr[1].z, in->arr[2].z,
                           in->arr[3].z } };
  out->zv = _mm_load_ps(loader.vec);

  loader = (vec_loader){ { in->arr[0].w, in->arr[1].w, in->arr[2].w,
                           in->arr[3].w } };
