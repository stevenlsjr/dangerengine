...
/**
 * GIVEN an iterator produced from this->table, table functions SHOULD:
 *   - have a valid key and value while iterator is active
 *   - have a key that produces a valid stl iterator in source std::map
 *   - have value equal to stl iterator's `val->second`
 *
 */
TEST_F(TableTests, TableIterate)
{
  slsHashItor _itor;
  auto itor = &_itor;
  for (sls_hashitor_first(this->table, itor); itor; itor = sls_hashitor_next(itor)) {
    auto const ptr = reinterpret_cast<int**>(itor->val);
    auto const key = *(reinterpret_cast<char **>(itor->key));
    EXPECT_TRUE(ptr && *ptr);
    auto found_pair = hash_data.find(key);
    auto num = **ptr;
    EXPECT_STREQ(key, found_pair->first.c_str());
    EXPECT_EQ(num, found_pair->second);
  }
}
